...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    console.log('🔧 Fixing order 194 - awarding missing points');
    ...
    if (existingTransaction.length === 0 && pointsToAward > 0) {
      ...
      const transaction = await sql`
        INSERT INTO points_transactions (
          ...
          'Points for Order #' || ${orderId} || ' (Authentication Fixed)', ${order[0].total}, NOW()
        ) RETURNING *
      `;
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          message: 'Order 194 points awarded - Authentication now working!',
          orderId: orderId,
          pointsAwarded: pointsToAward,
          transactionId: transaction[0].id,
          note: 'Authentication fix successful, manual points for this order'
        })
      };
    } else {
      return {
        ...
        body: JSON.stringify({
          success: true,
          message: 'Order 194 already has points',
          orderId: orderId,
          pointsAwarded: 0
        })
      };
    ...
    console.error('🔧 Fix order 194 error:', error);
    return {
      statusCode: 500,
      headers,
