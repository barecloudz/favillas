...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    console.log('🔧 Fixing order 191 user_id and awarding points');
    ...
    if (existingTransaction.length === 0 && pointsToAward > 0) {
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          message: 'Order 191 fixed and points awarded',
          orderId: orderId,
      ...
      return {
        ...
        body: JSON.stringify({
          success: true,
          message: 'Order 191 user_id fixed (points already existed)',
          orderId: orderId,
          pointsAwarded: 0
        })
      };
    ...
    console.error('🔧 Fix order 191 error:', error);
    return {
      statusCode: 500,
      headers,
